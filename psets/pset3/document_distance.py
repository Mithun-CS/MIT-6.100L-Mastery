...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    combined ={}
    for i in freq_dict1:
        combined[i] = freq_dict1[i]
    for i in freq_dict2:
        if i not in combined:
            combined[i] = freq_dict2[i]
        else:
            combined[i] += freq_dict2[i]

    max_freq = 0
    for i in combined:
        if combined[i] > max_freq:
            max_freq = combined[i]
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    """
    res_dict = {}
    raw_str = load_file(file_path)
    new_list = text_to_list(raw_str)
    dict_map = get_frequencies(new_list) 
    count = 0
    for i in dict_map.values():
        count += i
    for i in dict_map:
        res_dict[i] = dict_map[i]/count
    return res_dict
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    dictmap ={}
    for i in res_dict:
        dictmap[i] = math.log10(length/res_dict[i])
    return dictmap
