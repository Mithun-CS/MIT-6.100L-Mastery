...
def img_to_pix(filename):
    ...
    """
    my_image_variable = Image.open(filename)
    pixel_data = my_image_variable.getdata()
...
def pix_to_img(pixels_list, size, mode):
    ...
    """
    image_object = Image.new(mode,size)
    image_object.putdata(pixels_list)
    return image_object
...
def filter(pixels_list, color):
    ...
    for r,g,b in pixels_list:
      transformed = matrix_multiply([r,g,b],color_matrix)
      clean_pixel = tuple((int(round(float(x)))) for x in transformed)
      result.append(clean_pixel)
    return result
...
def reveal_bw_image(filename):
    ...
    """
    image_to_pixel = img_to_pix(filename)
    original_image = Image.open(filename)  
    size_image = original_image.size
    lst =[]
    for i in image_to_pixel:
        extracted_bit = (extract_end_bits(1,i))
        color_bits = extracted_bit*255
        lst.append(color_bits)
    hidden_image = pix_to_img(lst,size_image,'L')
    return hidden_image
...
def reveal_color_image(filename):
    ...
    Returns:
        result: an Image object containing the hidden image
    """
    image_to_pixels = img_to_pix(filename)
    original_image = Image.open(filename)
    size_image = original_image.size
    lst = []
    for i in image_to_pixels:
        extracted_bit = extract_end_bits(3,i)
        r_bits,g_bits,b_bits = extracted_bit
        color_bits = (int(r_bits*(255/7))) ,(int(g_bits*(255/7))), (int(b_bits*(255/7)))
        lst.append(color_bits)
    hidden_image = pix_to_img(lst,size_image,'RGB')
    return hidden_image
...
def main():
    ...
    # im2.show()
    im2.save('revealed_hidden.png')
    
    draw_kerb("revealed_hidden.png","MITHUN")
